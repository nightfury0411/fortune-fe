...
const PackageCard = ({ pkg, isBuy, handleBuyPackage, className, loading }) => {
  return (
    <div
      ...
      {loading ? (
        <>
          <div className="mb-6 flex flex-col items-center">
            ...
            <Skeleton.Input active className="!w-[300px] !h-[44px] mt-4" />
            {isBuy && (
              <Skeleton.Button active className="w-[140px] h-[44px] mt-5" />
            )}
          </div>

          <div className="!min-h-[200px] flex flex-col justify-start space-y-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <Skeleton.Input key={i} active className="!w-full" />
            ))}
          </div>
        </>
      ) : (
          ...
          <div className="flex flex-col items-center mb-6">
            ...
            <div className="h-20 flex flex-col items-center justify-center">
              <div className="text-3xl font-semibold text-primary text-center">
                {pkg?.price}
                <span className="block text-base font-medium">VNĐ/tháng</span>
              </div>
            </div>
            {isBuy && (
